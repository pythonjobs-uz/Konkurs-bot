...
class AnalyticsService:
    ...
    async def get_user_analytics(self, user_id: int) -> Dict[str, Any]:
        ...
        avg_participants = await self.db.execute(
            select(func.avg(func.count(Participant.id)))
            .join(Contest)
            .where(Contest.owner_id == user_id)
            .group_by(Contest.id)
        )
        
        max_participants = await self.db.execute(
            select(func.max(func.count(Participant.id)))
            .join(Contest)
            .where(Contest.owner_id == user_id)
            .group_by(Contest.id)
        )
        
        min_participants = await self.db.execute(
            select(func.min(func.count(Participant.id)))
            .join(Contest)
            .where(Contest.owner_id == user_id)
            .group_by(Contest.id)
        )
        ...
        analytics = {
            ...
            "avg_participants": float(avg_participants.scalar() or 0),
            "max_participants": max_participants.scalar() or 0,
            "min_participants": min_participants.scalar() or 0,
    ...
    async def get_system_analytics(self) -> Dict[str, Any]:
        ...
        weekly_growth = 0
        if users_week_ago.scalar() > 0:
            weekly_growth = ((total_users.scalar() - users_week_ago.scalar()) / users_week_ago.scalar()) * 100
        
        top_channel_result = await self.db.execute(
            select(Contest.channel_id, func.count(Participant.id).label('participants'))
            .join(Participant)
            .group_by(Contest.channel_id)
            .order_by(desc('participants'))
            .limit(1)
        )
        top_channel = top_channel_result.first()
        ...
        analytics = {
            ...
            "top_channel": top_channel.channel_id if top_channel else "N/A",
            "db_size": 0,
            "redis_memory": 0,
            "uptime": 24
        }
